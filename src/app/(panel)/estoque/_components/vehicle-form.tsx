...
export function VehicleForm({ initialData, submitAction, uploadAction, deleteAction }: { initialData?: any, submitAction: any, uploadAction: any, deleteAction?: any }) {
    ...
    const [formData, setFormData] = useState({
        ...
        condition: initialData?.condition || 'seminovo',
    })
    ...
    const BasicForm = (
        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-2"><label className="text-sm font-medium text-white/70">Condição</label><div className="relative"><select name="condition" value={formData.condition} onChange={handleBasicChange} className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-4 text-white appearance-none focus:outline-none focus:ring-2 focus:ring-[#FF4D00]/50"><option value="novo" className="bg-[#0A0A0A] text-white">Novo (0km)</option><option value="seminovo" className="bg-[#0A0A0A] text-white">Seminovo</option></select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/50 pointer-events-none" /></div></div>
